...
async fn yara_compile(path: String) {
    task::spawn_blocking(move || {
        log_message(LogLevel::None, format!("Compiling: {}", path).as_str(), false);
...
fn yara_scan_file(path: &str, rules: &[Arc<yara_x::Rules>]) {
    log_message(LogLevel::Warning, &format!("Scanning target: {}", path), false);
...
fn panic_scan(e: String) {
    ...
    log_message(LogLevel::Error, format!("{} Error:\n {}", msg, e).as_str(), false);
    panic!("{}", e.to_string());
}
...
async fn yara_scan_targets(paths: Vec<String>, rule_paths: Vec<String>, dirs: bool, memory: bool) {
    ...
    for rule_path in &rule_paths {
        ...
        log_message(LogLevel::Info, &format!("Reading: {}", compiled_path), false);

        match yara_rule_from_compiled(&compiled_path) {
            Ok(rule) => rules.push(rule),
            Err(e) => {
                log_message(LogLevel::Error, &format!("Failed to load rule: {}", e), false);
            }
        }
    }
